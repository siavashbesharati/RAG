...
import { ChatSession, QueryResult } from '../types';
...
export class ClientChatService {
  ...
  public async handleMessage(
    ...
    const matches: QueryResult[] = await this.vector.queryEmbedding(
      queryEmbedding,
      5
    );
  ...
  private async generateAnswer(
    ...
    const systemPrompt =
      'You are Quantivo Smart Support. Use the provided context to answer the question concisely.';
    ...
    if (userLang !== 'en') {
      answerInEnglish = await this.translation.translate(
        answerInEnglish,
        'en',
        userLang
      );

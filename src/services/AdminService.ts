...
export class AdminService {
  ...
  public async ingestText(
    ...
    const chunks = smartChunk(trimmed);
  ...
  private async embedChunks(chunks: string[]): Promise<number[][]> {
    ...
    for (let i = 0; i < chunks.length; i += BATCH_SIZE) {
      ...
      for (const d of sorted) {
        ...
        if (raw == null) throw new Error('Voyage API returned missing embedding');
        // Voyage may return Float32Array or number[]; ensure we have number[]
        let emb: number[];
        if (Array.isArray(raw)) {
          emb = raw as number[];
        } else if (typeof raw === 'object' && typeof (raw as any).length === 'number') {
          const len = (raw as any).length;
          if (len < 0 || len > 10000 || !Number.isFinite(len)) {
            throw new Error('Voyage API returned embedding with invalid length');
          }
          emb = [];
          for (let j = 0; j < len; j++) {
            const v = (raw as any)[j];
            if (typeof v !== 'number') throw new Error('Voyage API returned invalid embedding values');
            emb.push(v);
          }
        } else {

...
// initialise services
const vectorService = new VectorService_1.VectorService(PINECONE_KEY, PINECONE_ENV, PINECONE_INDEX);
const adminService = new AdminService_1.AdminService(vectorService, VOYAGE_KEY);
const translationService = new TranslationService_1.TranslationService(GEMINI_KEY);
const clientChatService = new ClientChatService_1.ClientChatService(translationService, vectorService, VOYAGE_KEY);
...
async function main() {
    ...
    // simulate a chat
    const answer = await clientChatService.handleMessage('session-123', 'سلام، چگونه می‌توانم حساب خود را اصلاح کنم؟' // Persian sample
    );
